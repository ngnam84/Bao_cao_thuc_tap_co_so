...
const Suppliers = () => {
    ...
    // Hàm mở modal chỉnh sửa khi nhấn nút "Chỉnh sửa"
    const handleEditSupplier = async (supplierId) => {
        setLoading(true);
        setId(supplierId);
        try {
            // Lấy thông tin nhà cung cấp cần chỉnh sửa
            await supplierApi.getDetailSupplier(supplierId).then((res) => {
                console.log("Supplier detail:", res);
                form2.setFieldsValue({
                    name: res.name,
                });
                setDescription(res.description);
                setUploadFile(res.image);
                setLoading(false);
                setOpenModalUpdate(true);
            });
        } catch (error) {
            console.log('Failed to fetch supplier details:', error);
            setLoading(false);
        }
    };
    ...
    const handleUpdateCategory = async (values) => {
        ...
        try {
            ...
            // Sử dụng axiosClient.put trực tiếp vì supplierApi.updateSupplier gặp lỗi
    ...
    const handleDeleteCategory = async (id) => {
        ...
        try {
            await supplierApi.deleteSupplier(id).then(response => {
                if (response.error === "Cannot delete supplier. There are products associated with this supplier.") {
                    setLoading(false);
                    return notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Không thể xóa nhà cung cấp. Vì sản phẩm đang chứa nhà cung cấp',
